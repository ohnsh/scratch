// adapted from: https://github.com/tsl0922/ttyd/blob/main/html/src/components/terminal/xterm/index.ts
...
import { OverlayAddon } from './addon-overlay.ts';
...
export class Xterm {
    ...
    public sendFile(files: FileList) {
        throw "zmodem sendFile() not implemented."
    ...
    private parseOptsFromUrlQuery(query: string): Preferences {
        ...
        const queryObj = Array.from(new URLSearchParams(query) as unknown as Iterable<[keyof Preferences, string]>);

        if (!terminal) {
            throw new Error('terminal is not initialized');
        }
        for (const [k, queryVal] of queryObj) {
            /*const validKey: (key) => key is keyof Preferences =
            key => ['rendererType', 'disableLeaveAlert', 'disableResizeOverlay',
                'enableZmodem', 'enableTrzsz', 'enableSixel', 'titleFixed', 'isWindows',
                'trzszDragInitTimeout', 'unicodeVersion', 'closeOnDisconnect'
            ].includes(key)

            if (!validKey(k)) {
                console.warn(`[ttyd] unknown option in url query: ${k}=${queryVal}, ignoring`);
                continue;
            }*/
            ...
            let v = clientOptions[k as keyof ClientOptions];
            if (v === undefined) v = terminal.options[k as keyof ITerminalOptions];
            switch (typeof v) {
                case 'boolean':
                    setPref(k, queryVal === 'true' || queryVal === '1' as never);
                    break;
                case 'number':
                case 'bigint':
                    setPref(k, Number.parseInt(queryVal, 10));
                    break;
                case 'string':
                    setPref(k, queryVal);
                    break;
                case 'object':
                    setPref(k, JSON.parse(queryVal));
                    break;
                default:
    ...
    private applyPreferences(prefs: Preferences) {
        ...
        type PrefsEntry = Iterable<any> & { [K in keyof Preferences]: [K, Preferences[K]] }[keyof Preferences];
        for (const [key, value] of Object.entries(prefs) as Iterable<PrefsEntry>) {
            switch (key) {
                case 'rendererType':
                ...
                default:
                    ...
                    if (terminal.options[key as keyof ITerminalOptions] instanceof Object) {
